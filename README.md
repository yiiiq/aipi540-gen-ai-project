...
- Before/after comparison: generated by `scripts/evaluate_model.py`
- Live app: Streamlit app in `main.py`
- Inference on trained model: app loads `models/medexplain-lora/`
- Good UX: paste text, choose a sample, compare base vs adapted output
- Risks and ethics: hallucination, medical safety, bias in training examples, accessibility
- Code quality: modular package under `src/medexplain`
- Dataset script: `scripts/make_dataset.py`
- Training script: `scripts/train_model.py`
- Prediction/evaluation script: `scripts/evaluate_model.py`
- Notebooks isolated: `notebooks/`
- GitHub workflow: use feature branches and pull requests before merging to `main`

## Suggested Pitch Outline

1. Problem: medical text is often hard for patients to understand.
2. Model: fine-tuned `flan-t5-small` with LoRA.
3. Data: paired examples of medical language and patient-friendly rewrites.
4. Demo: show base model vs fine-tuned model on the same input.
5. Evaluation: readability and jargon reduction.
6. Risks: not a diagnosis tool, possible hallucination, need clinician review for real use.
